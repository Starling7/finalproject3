...
with conn.session as session:
    query = text('CREATE TABLE IF NOT EXISTS TICKETS (id serial, class varchar, supporter_name varchar, gender char(25), \
...
if page == "Edit Data":
    if st.button('Tambah Data'):
        with conn.session as session:
            query = text('INSERT INTO tickets (class, supporter_name, gender, stadium_name, ticket_price, match_name, time_info, date_info) \
    ...
    for _, result in data.iterrows():        
        ...
        with st.expander(f'a.n. {supporter_name_lama}'):
            with st.form(f'data-{id}'):
                ...
                class_baru = st.selectbox("class", list_class, list_class.index(class_lama))
                    ...
                    if st.form_submit_button('UPDATE'):
                        with conn.session as session:
                            query = text('UPDATE tickets \
                                          SET class=:1, supporter_name=:2, gender=:3, stadium_name=:4, \
                                          ticket_price=:5, match_name=:6, time_info=:7, date_info=:8 \
                                          WHERE id=:9;')
                            session.execute(query, {'1':class_baru, '2':supporter_name_baru, '3':gender_baru, '4':str(stadium_name), 
