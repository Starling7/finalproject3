...
list_tribune = ['', 'Economy', 'VIP', 'VVIP', 'Reguler']
...
if page == "Edit Data":
    if st.button('Tambah Data'):
        with conn.session as session:
            query = text('INSERT INTO tickets (tribune_name, supporter_name, gender, stadium_name, ticket_price, match_name, time_info, date_info) \
                    ...
                    if st.form_submit_button('UPDATE'):
                        with conn.session as session:
                            query = text('UPDATE tickets \
                                          ...
                                          WHERE id=:9;')
                            session.execute(query, {'1':tribune_name_baru, '2':supporter_name_baru, '3':gender_baru, '4':str(stadium_name), 
